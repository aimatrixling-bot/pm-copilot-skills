...
工程需求必须回溯到技术规格中的约束 ID，确保无约束遗漏。

```
Tech Spec C-[NNN] → Eng Request 验收标准/技术约束 → 开发实现
       (来源)              (翻译)                    (执行)
```

**回溯规则**：
- 每个工程需求必须覆盖 ≥1 条 C-[NNN] 约束
- 验收标准中必须体现约束的验证方法
- 如果某约束无法分配到任何工程需求 → 创建专门的约束追踪项并标注 [待分配]

## 执行流程

```
1. Read PRD 提取功能范围和验收标准
2. Read 技术规格提取约束翻译矩阵（如有）
   ├── 收集所有 C-[NNN] 约束 ID
   └── 理解每条约束的技术方案和验证方法
3. 生成工程需求单
   ├── 背景（为什么做）—— 自包含，不依赖 PRD
   ├── 需求描述（做什么）—— 具体到可执行
   ├── 验收标准（Given-When-Then）—— 可直接测试，标注覆盖的 C-[NNN]
   ├── 技术约束（非功能需求）—— 性能/安全/兼容，标注来源 C-[NNN]
   ├── 依赖（外部系统/团队）—— 已沟通/待沟通
   └── 估时建议（粗估）—— 标注置信度
4. 约束覆盖检查
   ├── 所有 C-[NNN] 是否已分配到某个需求？
...
## 验收标准
- [ ] Given [前置条件] When [操作] Then [预期结果] `(C-[NNN])`
- [ ] Given [前置条件] When [操作] Then [预期结果] `(C-[NNN])`
- [ ] Given [异常场景] When [操作] Then [预期错误处理] `(C-[NNN])`
