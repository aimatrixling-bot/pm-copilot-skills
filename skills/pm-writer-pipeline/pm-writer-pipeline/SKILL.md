...
PM 文档不是学术论文，不是技术博客，不是营销文案。语调必须统一。

### 核心语调

| 维度 | 正确 | 错误 |
| --- | --- | --- |
| 精确 | "DAU 从 1.2 万降至 0.8 万" | "用户活跃度有所下降" |
| 直接 | "不做 X，因为 Y" | "经过综合考量，我们建议暂缓 X 的推进" |
| 可执行 | "前端 3 天内完成列表页开发" | "前端页面需要尽快推进" |
| 诚实 | "数据不足，暂不推荐" | "基于初步分析，该方向具有一定潜力" |

### 禁词表（AI 味表达）

以下表达在所有文档中禁止使用，替换为具体内容：

| 禁止 | 替换为 |
| --- | --- |
| "值得注意的是" | 删除，直接陈述 |
| "综上所述" | 删除，结论前置 |
| "在当今数字化时代" | 删除，直接说问题和方案 |
| "至关重要" / "不可或缺" | 具体说明不做的后果 |
| "旨在" | "目的是" 或直接写目的 |
| "赋能" / "助力" | 具体说帮谁做什么 |
| "全方位" / "一站式" / "一体化" | 列出具体包含什么 |
| "确保"（无动作） | 写具体怎么确保 |
| "持续优化" | 写优化什么、到什么指标 |
| "加强" / "提升"（无量化） | 写具体提升什么指标多少 |
| "有助于" / "有利于" | 写因果链条 |

### 一致性规则

管道内所有文档必须：
- **术语一致**：同一概念只用一个词（不混用"用户"/"客户"/"使用者"）
- **数字一致**：路线图/PRD/发布计划中的同一指标数值相同
- **范围一致**：In Scope 边界在各文档中不矛盾
- **语调一致**：全部使用上述核心语调，不因文档类型切换风格
