...
```yaml
review_mode: contract_review | evidence_review | design_review | prototype_design_evidence_review | prototype_to_spec_review | release_readiness | not_reviewable
review_target:
contract_checked:
findings:
evidence_audit:
...
design_plan_audit:
prototype_design_evidence_audit: not_applicable | findings
prototype_to_spec_audit: not_applicable | findings
...
- Prototype design evidence review 必须覆盖 source visual、rendered implementation、preview URL、screenshot path、viewport/state、design QA 和 P0-P3 findings。
- Prototype-to-spec review 必须覆盖 source provenance、prototype facts vs inferred requirements、prototype gaps、mock boundary、verification provenance 和 spec-first guard。
